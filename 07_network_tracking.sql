...
CREATE TABLE qgep_network.node (
  ...
  ne_id TEXT NULL REFERENCES qgep_od.wastewater_networkelement(obj_id) ON UPDATE CASCADE ON DELETE CASCADE, -- reference to the network element (this will reference the reach object for reachpoints)
  rp_id TEXT NULL REFERENCES qgep_od.reach_point(obj_id) ON UPDATE CASCADE ON DELETE CASCADE, -- will only be set for reachpoints
...
CREATE TABLE qgep_network.segment (
  ...
  from_node INT REFERENCES qgep_network.node(id) ON UPDATE CASCADE ON DELETE CASCADE,
  to_node INT REFERENCES qgep_network.node(id) ON UPDATE CASCADE ON DELETE CASCADE,
  ne_id TEXT NULL REFERENCES qgep_od.wastewater_networkelement(obj_id) ON UPDATE CASCADE ON DELETE CASCADE, -- reference to the network element (will only be set for segments corresponding to reaches)
